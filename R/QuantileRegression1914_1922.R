function_quantile_regression_1914_1922 <- function(varExp) {
  ...
  else if( varExp == "unadjusted_married_linear") {
    formula<-as.formula( paste("weight ~ married"))
    qr1 <- lm(formula , data=datared)
    summary(qr1)
  }
  ...
  else if( varExp == "unadjusted_insurance_linear") {
    formula<-as.formula( paste("weight ~ insurance"))
    qr1 <- lm(formula , data=datared)
    summary(qr1)
  }
  ...
  else if( varExp == "unadjusted_flu_qr") {
    
    formula<-as.formula( paste("weight ~ Flu_intensity_all"))
    qr1 <- rq(formula , data=datared , tau =c(0.1, 0.5, 0.9))
...
# adjusted
  ...
  else if( varExp == "adjusted_year_linear_plot") {
    
    qr1 <- lm(weight ~ year + boy+parity+Gest_group+birth_month+matage+ married+city +insurance, data=datared)
    qr1_sum <- summary(qr1)
    
    qr1_results <- data.frame(qr1_sum$coefficients)[2:9,] %>%
      mutate( CIl = Estimate-1.96*Std..Error,
              CIu = Estimate+1.96*Std..Error,
              Year = row.names(.)) %>%
      select(Year, Estimate, CIl, CIu) %>%
    ...
    formula<-as.formula( paste("weight ~ year + boy+parity+Gest_group+birth_month+matage+ married+city +insurance"))
    qr1 <- rq(formula , data=datared , tau =c(0.1, 0.5, 0.9))
    ...
    qr1_std_error01 <- qr1_sum[[1]][3]$coefficients[,2][2:9] %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.1)
    
    qr1_std_error05 <-qr1_sum[[2]][3]$coefficients[,2][2:9]  %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.5)
    
    qr1_std_error09 <-qr1_sum[[3]][3]$coefficients[,2][2:9] %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.9)
  ...
  else if( varExp == "adjusted_flu_qr") {
    
    formula<-as.formula( paste("weight ~ Flu_intensity_all + boy+parity+Gest_group+birth_month+matage+ married+city +insurance"))
    qr1 <- rq(formula , data=datared , tau =c(0.1, 0.5, 0.9))
    ...
    qr1_std_error01 <- qr1_sum[[1]][3]$coefficients[,2][2] %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.1)
    
    qr1_std_error05 <-qr1_sum[[2]][3]$coefficients[,2][2]  %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.5)
    
    qr1_std_error09 <-qr1_sum[[3]][3]$coefficients[,2][2] %>%
      as.data.frame() %>%
      rename(sd_error = ".") %>%
      mutate(Variables = row.names(.),
             quantile = 0.9)
    ...
    data_qr1 <- qr1$coefficients[2:3,]  %>%
      as.data.frame()%>%
      mutate(Variables=row.names(.)) %>%
      filter(!Variables=="boy0") %>%
      gather(., condition, measurement, `tau= 0.1`:`tau= 0.9`,factor_key=TRUE)%>%
      mutate(quantile = as.numeric(substr(condition,6,8))) %>%
      select(-condition) %>%
      left_join(qr1_std_error) %>%
